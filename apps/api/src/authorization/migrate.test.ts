...
test("keeps the RBAC schema, catalog, and system-role safeguards in the authorization migration", async () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      "0009-authorization-activities-permissions.sql",
    ],
  );
...
test("adds idempotent Requirement permissions with the agreed system-role grants", async () => {
  const migrations = await readAuthorizationMigrations();
  const migration = migrations.find(
    (candidate) => candidate.name === "0005-authorization-requirements-permissions.sql",
  );
...
test("adds idempotent Activity permissions with the agreed system-role grants", async () => {
  const migrations = await readAuthorizationMigrations();
  const migration = migrations.find(
    (candidate) => candidate.name === "0009-authorization-activities-permissions.sql",
  );
